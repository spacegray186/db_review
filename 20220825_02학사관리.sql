...
-- tb_student 테이블 전체 레코드 개수
select count(*) from tb_student;    -- 6

-- tb_gwamok 테이블 전체 레코드 개수
select count(*) from tb_gwamok;     -- 9

-- tb_sugan 테이블 전체 레코드 개수
...
문2)학생테이블에서 동명이인이 각 몇 명인지 조회하시오
select uname, count(*)
from tb_student
where uname=uname
group by uname;
...
문4)학생테이블에서 주소별 인원수가 3명 미만 행을 조회하시오
select address, count(*)
from tb_student
group by address
having count(*)<3;
