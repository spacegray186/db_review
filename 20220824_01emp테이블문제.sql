...
insert into emp(empno, ename, job, mgr, hiredate, sal, deptno)
values(7654, '손흥민', '사원', 7839, '2002-01-11', 400, 30);

insert into emp(empno, ename, job, mgr, hiredate, sal, deptno)
values(7698, '박지성', '주임', 7698, '2000-07-12', 325, 20);

insert into emp(empno, ename, job, mgr, hiredate, sal, deptno)
values(7782, '김연아', '과장', 7698, '2001-12-17', 225, 10);
...
insert into emp(empno, ename, job, mgr, hiredate, sal, deptno)
values(7876, '정우성', '사원', 7839, '2004-09-09', 500, 30);

insert into emp(empno, ename, job, mgr, hiredate, sal, deptno)
values(7900, '김혜수', '대표', 7902, '2001-12-03', 200, 20);
////////////////////////////////////////////////////////////////////////////////
...
select ename, deptno, sal, comm, 연봉
from emp;

alter table emp add 연봉 number(7,2);
...
문24) 커미션이 null이면 0으로 바꾼 후 연봉을 다시 계산해서
      이름, 급여, 커미션, 연봉을 조회하시오
select ename, sal, nvl(comm, 0), 연봉
from emp;

alter table emp add 연봉 number(7,2);

update emp set 연봉=(sal*12)+nvl(comm, 0);
...
문25) 각 사람의 급여를 검색해서 '누구누구의 급여는 얼마입니다'로 조회하시오
      (|| 결합연산자)
select ename || '의 급여는 ' || sal || '만원 입니다' from emp;
select ename || '의 급여는 ' || sal || '만원 입니다' as pay from emp;
...
문29) 다음의 SQL문을 분석하시오
      ...
      order by sal;
나 → 급여가 200초과 300미만이 아닌 사원들의 사번과 급여를 급여순으로 조회하시오

문30) 다음의 SQL문을 분석하시오
      select empno, sal 
      from emp
      where not sal>200 and sal<300     -- 200이하이면서 300미만인 행 조회
      order by sal;
나 → 급여가 200을 초과하지 않고 300미만인 사원의 사번과 급여를 급여순으로 조회하시오
...
문33) 부서별 급여평균이 300이상 조회하시오 (전체 행을 대상으로)
...
문34) 급여가 300이상 데이터 중에서 부서코드별 급여평균을 구해서 급여평균순으로 조회하시오
select deptno, avg(sal)
from emp
where sal>=300
group by deptno
...
문35) 급여가 300이상 데이터 중에서 부서코드별 급여평균이 400이상을 급여순으로 조회하시오
select deptno, avg(sal)
from emp
where sal>=300
group by deptno
...
select to_char(hiredate, 'yy') as 입사연도 from emp;
...
문40) 각 직원들에 대해서 직원의 이름과 근속 연수를 구하시오
      단, 근속 연수는 연단위를 버림하여 나타내시오
...
select ename, hiredate, trunc((sysdate-hiredate)/365) as 근속연수 from emp;
...
문43) 입사한지 만 15년 이상된 사람에 한해 현재연봉에서 10% 인상시켰을 때 
      ...
      연봉구하는 식 : 급여(sal)*12개월+보너스(comm)
select empno, ename, hiredate, (sal*12+nvl(comm,0)) as 현재연봉, (sal*12+nvl(comm,0))*1.1 as 인상후연봉, ((sal*12+nvl(comm,0))*1.1)-(sal*12+nvl(comm,0)) as 인상된금액
from emp
